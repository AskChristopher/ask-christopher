...
- [ ] **Eval suite — built, and now measured end to end.** Every scoring path exists and every case has an outcome as of the 2026-08-29 baseline checkpoint; what remains open is acting on the two failures it found. `tests/evals/cases.yaml` holds 40 cases across the seven categories, with eight tradeoffs guarded in both directions, and `src/ask_christopher/evals.py` scores them against any injected response function.
  ...
  - [ ] **Read the two baseline failures, then choose one targeted Phase 1 improvement.** The failure map is in the checkpoint and is deliberately uninterpreted.
