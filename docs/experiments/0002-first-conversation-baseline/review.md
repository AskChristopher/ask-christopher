...
- ~~**Correction handling untested** in both directions.~~ **Addressed elsewhere.** Both halves of the pair were run single-turn through the eval suite on 2026-08-12 and judged to pass — see [`docs/evals/correction-pair-review.md`](../../evals/correction-pair-review.md). `crn-valid-correction` supplies the prior claim inside its own prompt, so it never needed a conversation. It remains untested in the shape this experiment intended: a correction of something the assistant actually said, with its own prior turn in context.
